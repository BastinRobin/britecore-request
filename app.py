...
def check_if_priority_already_exist(priority, client):
    ...
    return Feature.query.filter_by(priority=priority,
                                   client=client).count()
...
def get_existing_priority_record(priority, client):
    ...
    return Feature.query\
        .filter_by({'priority': priority, 'client': client}).first()
...
def feature():
    ...
    if request.method == 'POST':
        ...
        if check_if_priority_already_exist(request.form['priority'],
                                           request.form['client']) > 0:

            # fetch entire task for the client

            features = Feature.query.\
                filter_by(client=request.form['client']).\
        ...
        target_date = datetime\
            .strptime(request.form['target_date'], '%Y-%m-%d')
...
def set_priority():
    ...
    if request.method == 'POST':
        ...
        if request.form['mode'] == 'up':
            ...
            previous_row = \
                Feature.query.filter_by(client=request.form['client'])\
